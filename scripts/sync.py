...
if not EVENT_PATH:
    EVENT_PATH = os.environ.get("GITHUB_TEST_EVENT_PATH")
    raise RuntimeError(
...
def extract_events(text, fallback_date, source, source_id):
    ...
    if m:
        date = parse_date(m.group(1), format="%m%d%Y").date()
        page = int(m.group(2))
        events.append({
            "page": page,
            "date": date,
...
comments_resp = requests.get(issue["comments_url"],headers=headers).json()
